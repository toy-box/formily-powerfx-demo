...
export const TableStatusBar: FC<ITableStatusBarProps> = ({ viewSetter = true, selectStatus = true }) => {
      ...
      <>
        <div>
          {dataGrid.selectionType && (
            <span>
              已经选择 {(dataGrid.selectedKeys || []).length} 条记录
            </span>
          )}
        </div>
      </>
    ),
    [dataGrid.selectionType, dataGrid.selectedKeys]
  )
