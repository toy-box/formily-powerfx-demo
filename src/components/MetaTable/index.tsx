...
const useSelectedKeys = (): [string[], (keys?: string[]) => void] => {
  ...
  const { form } = field
  const path = field.parent.path.concat('selectedKeys')
  return [
  ...
  const parseSources = (schema: Schema): ObservableColumnSource[] => {
    if (
      ...
      const field = arrayField
        .query(arrayField.address.concat(name)).take()
      const columnProps =
...
export const MetaTable: ComposedArrayTable = observer(
  (props: IMetaTableProps<any>) => {
    ...
    const rowSelection = useMemo(
      () =>
        props.defaultSelectionType != null
          ? {
              type: props.defaultSelectionType,
              selectedKeys: dataGrid.selectedKeys,
              onChange: (keys: string[], rows: Array<Record<string, any>>) => {
                if (props.keepSelected) {
                  dataGrid.setSelectedKeys(
                    dataGrid.selectedKeys
                      .filter(
                        (key) =>
